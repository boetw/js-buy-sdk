...
suite('query-test', () => {
  ...
  test('it creates product queries with specified fields', () => {
    ...
    const queryString = 'query { shop { products (first: 20) { pageInfo { hasNextPage,hasPreviousPage },edges { cursor,node { id,tags,vendor,images (first: 20) { pageInfo { hasNextPage,hasPreviousPage },edges { cursor,node { src } } },options { id,name },variants (first: 20) { pageInfo { hasNextPage,hasPreviousPage },edges { cursor,node { id,title } } } } } } } }';
  ...
  test('it creates collection queries with defaults', () => {
    ...
    const queryString = 'query { shop { collections (first: 20) { pageInfo { hasNextPage,hasPreviousPage },edges { cursor,node { id,handle,updatedAt,title,image { id,src,altText } } } } } }';
  ...
  test('it creates collection queries with specified fields', () => {
    ...
    const queryString = 'query { shop { collections (first: 20) { pageInfo { hasNextPage,hasPreviousPage },edges { cursor,node { id,handle,updatedAt,title,image { id } } } } } }';
